  ...
  return (
    <div className="flex flex-row content-between w-full px-3 py-5 mt-5 bg-white border mt border-primaryBorder shadow-default">
      <div className="w-1/12">
        <p>{moment(booking.booking.date).format("dddd, Do MMM")}</p>
        <p className="text-gray-400">
          {moment(booking.booking.date).format("HH:mm") +
            " - " +
            moment(booking.booking.date).add(15, "minutes").format("HH:mm")}
        </p>
      </div>
      <div className="w-7/12 ml-8">
        <p className="font-bold">{booking.booking.title}</p>
        <p className="text-gray-400">{booking.booking.description}</p>
